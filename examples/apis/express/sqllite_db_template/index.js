...
// Inicializamos una base de datos local con sqlite3
...
app.post("/post", (req, res) => {
  ...
  db.run(
    "INSERT INTO posts (titulo, descripcion, imagen, fecha, categoria) VALUES (?, ?, ?, ?, ?)",
    [titulo, descripcion, imagen, fecha, categoria],
    function (err) {
      if (err) {
        res.status(500).json({ error: err.message });
        return;
...
app.post("/comment", (req, res) => {
  ...
  db.run(
    "INSERT INTO comments (post_id, username, comentario, fecha) VALUES (?, ?, ?, ?)",
    [post_id, username, comentario, new Date().toISOString()],
    function (err) {
      if (err) {
        res.status(500).json({ error: err.message });
        return;
